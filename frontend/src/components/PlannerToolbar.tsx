...
type OpenMenuId = 'filter' | 'customize' | null;
...
const PlannerToolbar: React.FC<PlannerToolbarProps> = ({
  ...
  return (
    <div
      className={`relative overflow-visible border-b border-slate-200 bg-white px-4 py-3 ${
        fillHeight ? 'flex h-full flex-col justify-between' : ''
      }`}
    >
      <div
      ...
      />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-white via-white/85 to-white/72" />

      <div className="relative z-10 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Planner</p>
        </div>
        {onToggleAgenda && (
          <button
            type="button"
      ...
      <div ref={menuBarRef} className="relative z-10 mt-2 flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-1 flex-wrap items-center gap-2">
          {viewMode && onViewModeChange && (
            <div className="flex items-center rounded-full border border-gray-200 bg-white/95 p-1 shadow-sm">
              <button
                type="button"
                onClick={() => onViewModeChange('list')}
                className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm transition-colors ${
                  viewMode === 'list' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-slate-50 hover:text-gray-800'
                }`}
              >
                <List size={14} />
                List
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm transition-colors ${
                  viewMode === 'gantt' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-slate-50 hover:text-gray-800'
                }`}
              >
                <BarChart2 size={14} />
                Gantt
          ...
          >
            <ToolbarMenuButton
              ...
              label={
                hasActiveFilters
                  ? `Filter · ${selectedStatuses.length > 0 ? `${selectedStatuses.length} status` : 'Active'}`
                  : 'Filter'
              }
              onClick={() => openHoverMenu('filter')}
            />

            {openMenu === 'filter' && (
              <div className="absolute left-0 top-full z-50 mt-2 max-h-[68vh] w-64 overflow-y-auto rounded-[1rem] border border-slate-200 bg-white p-2 shadow-2xl">
          ...
          <div
            className="relative"
            onMouseEnter={() => openHoverMenu('customize')}
            onMouseLeave={() => scheduleMenuClose('customize')}
          >
            <ToolbarMenuButton
              active={openMenu === 'customize'}
              label="Customize"
              icon={<Settings2 size={14} />}
              onClick={() => openHoverMenu('customize')}
            />
            {openMenu === 'customize' && (
              <div className="absolute left-0 top-full z-50 mt-2 w-72 rounded-[1rem] border border-slate-200 bg-white p-3 shadow-2xl">
                {onToggleDefaultTaskTreeExpanded && (
                  <div className="rounded-[1rem] border border-slate-200 bg-slate-50/70 p-1">
                    <SwitchRow
                      label="Tasks open by default"
                      description="Start project task trees expanded in the left sidebar."
                      checked={defaultTaskTreeExpanded}
                      onToggle={onToggleDefaultTaskTreeExpanded}
                    />
                  </div>
                )}

                {(savedViews.length > 0 || onSavedViewSelect || onSaveCurrentView) && (
                  <div className="mt-3">
                    <p className="px-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Saved views</p>
                    <div className="mt-2 space-y-1">
                      {savedViews.length > 0 ? (
                        savedViews.map((view) => (
                          <button
                            ...
                            className={`flex w-full items-center justify-between rounded-xl px-2 py-1.5 text-left text-xs transition-colors ${
                              activeSavedViewId === view.id ? 'bg-blue-50 text-blue-700' : 'text-slate-700 hover:bg-slate-50'
                            }`}
                          >
                            <span>{view.name}</span>
                            {activeSavedViewId === view.id && <Check size={14} />}
                          </button>
                        ))
                      ) : (
                      ...
                      {onSavedViewSelect && (
                        ...
                        >
                          ...
                          {!activeSavedViewId && <Check size={14} />}
                        </button>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-3 flex gap-2 border-t border-slate-100 pt-3">
                  {onSaveCurrentView && (
                    <button
                      ...
                      className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 transition-colors hover:bg-slate-50"
                    >
                      Save current
                    </button>
                  )}
                  <button
                    ...
                    title={viewBuilderTitle}
                    className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Open panel
                  </button>
                </div>
      ...
      {subControls && <div className="relative z-10 mt-2 flex flex-wrap items-center gap-2">{subControls}</div>}
    </div>
  );
};
